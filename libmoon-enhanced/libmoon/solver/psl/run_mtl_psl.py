...
if __name__ == '__main__':
    ...
    parse.add_argument('--solver', type=str, default='epo')
    ...
    loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=args.batch_size, shuffle=True,
                                              num_workers=0)
    args.device = torch.device("cpu") if args.device_name == 'cpu' else torch.device("mps")
    if args.model == 'lenet':
        hnet = HyperNet([9, 5])
        net = LeNetTarget([9, 5])
    else:
    ...
    for idx in tqdm(range(args.n_epoch)):
        for i, batch in enumerate(loader):
            ...
            loss_arr = torch.stack([loss(logits, **batch_) for (loss, logits) in zip(loss_func_arr, logits_l)])
            optimizer.zero_grad()
