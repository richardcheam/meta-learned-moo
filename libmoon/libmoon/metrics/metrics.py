...
def compute_FD(sols,pareto_points):
    FD = 0
    for sol in sols :
        min = np.inf
        for pareto_point in pareto_points :
            if np.linalg.norm(sol - pareto_point) < min :
                min = np.linalg.norm(sol - pareto_point)
        FD += min
    return FD


def compute_indicators(objs, prefs, problem_name='ZDT1', pareto_points = None):
    '''
        Input:
            objs: objective arrays
    ...
    if pareto_points is not None :
        FD = compute_FD(sols=objs, pareto_points=pareto_points)
        return {
        'lmin': lmin,
        'soft_lmin': soft_lmin,
        'spacing': spacing,
